...
def waktu():
	now = datetime.now()
	hours = now.hour
	if 4 <= hours < 12:timenow = "Selamat Pagi di script Alif"
	elif 12 <= hours < 15:timenow = "Selamat Siang di script alif"
	elif 15 <= hours < 18:timenow = "Selamat Sore di script alif"
	else:timenow = "Selamat Malam tunda dulu main termuxnya bro"
	return timenow
...
def menuku():
	...
	_____Alif_____ = input(f'* pilih : ')
	if _____Alif_____ in ['1']:Nomor()
	elif _____Alif_____ in ['2']:Result()
	else:
...
def Results():
	...
	if kz in ['02','2']:
		...
		print(f' {P}└─{J} pilih yang bener kuy ')
		back()
def Nomor():
	print()
